...
> 版本：3.3.0 ｜ 文档日期：2026-09-06
> 说明：本文档基于当前代码实际实现（`src/manifest.json` 版本 3.3.0）整理，作为产品功能、交互与约束的完整参考。
...
- **域名黑名单**：每行一个域名，命中后插件在该站点完全不生效；黑名单优先于白名单
- **规则解析追加模式**：开启后，内容面板执行“规则重新解析”时，新识别题目会追加到现有题目列表下方
...
- **规则重新解析追加**：开启“规则解析追加模式”后，重新解析仅追加未出现的新题，避免覆盖已有题目列表
- **AI 选区解析**：鼠标悬停高亮可选区域，点击后 AI 解析该区域题目，可自动生成/合并解析规则
- **面板操作**：拖动、最小化为悬浮条、关闭
- **快捷键唤起**：默认 `Alt+Q`，通过白名单/黑名单校验后触发；在输入框、`contenteditable`、Monaco、CodeMirror 等可编辑区域，以及输入法组合态下不响应，避免误触
...
| `allowed_domains` / `blocked_domains` | 生效域名白名单与黑名单 |
| `panel_shortcut` / `theme_mode` / `rule_parse_append_mode` | 快捷键、主题与规则解析追加模式 |
