...
export async function checkMonthlySearchLimit(providerId) {
  const result = await chrome.storage.local.get([STORAGE_KEYS.WEB_SEARCH_PROVIDERS, STORAGE_KEYS.WEB_SEARCH_USAGE]);
  ...
  const usage = result[STORAGE_KEYS.WEB_SEARCH_USAGE] || {};
...
export async function incrementMonthlySearchCount(providerId) {
  const result = await chrome.storage.local.get([STORAGE_KEYS.WEB_SEARCH_USAGE]);
  const usage = result[STORAGE_KEYS.WEB_SEARCH_USAGE] || {};
  const record = getOrResetProviderUsage(usage, providerId);
  record.count += 1;
  usage[providerId] = record;
  await chrome.storage.local.set({ [STORAGE_KEYS.WEB_SEARCH_USAGE]: usage });
}
