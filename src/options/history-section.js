...
function initHistory({ historyListEl, onOpenDrawer }) {
  ...
  function renderHistory(history) {
    if (history.length === 0) {
      historyListEl.innerHTML = '<div class="list-empty">' + getMessage('optionsHistoryEmpty') + '</div>';
      return;
    }
      ...
      item.innerHTML = `
        <div class="list-item-header">
          <div class="list-item-info">
            <div class="list-item-title">${escapeHtml(record.title || getMessage('optionsHistoryUnnamed'))}</div>
            <div class="list-item-meta">${getMessage('optionsHistoryMetaFormat', [date, record.questions.length, doneCount])}${errorCount > 0 ? getMessage('optionsHistoryErrorSuffix', [errorCount]) : ''}</div>
  ...
  async function deleteHistoryItem(idx) {
    if (!confirm(getMessage('optionsHistoryDeleteConfirm'))) return;
    const result = await chrome.storage.local.get(['exam_history']);
  ...
  function exportSingleHistory(record) {
    ...
    a.href = url;
    const title = (record.title || getMessage('optionsHistoryUnnamed')).replace(/[^\w\u4e00-\u9fa5]/g, '_').slice(0, 30);
